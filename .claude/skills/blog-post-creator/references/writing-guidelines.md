...
标题的唯一任务是让人想点开。它不应该是文章结论的压缩版——结论留给正文。

**5 种句式轮换（连续两篇不得使用同一种）：**

| 句式 | 机制 | 示例 |
|------|------|------|
| **数字+悬念** | 具体数字引发"怎么做到的？" | 两小时，一个 Sonnet，七万星仓库的完整移植 |
| **悖论/反问** | 认知冲突引发"为什么？" | 为什么你的 Agent 知道得越多，表现越差 |
| **事实+意外** | 简单事实，意外结果 | 同一个模型，换套壳，排名从 Top 30 跳到 Top 5 |
| **数字+极简** | 一个数字讲完整个故事 | 一个 Markdown 文件让 Agent 快了 28% |
| **类比/场景** | 画面感或熟悉场景的迁移 | Agent 和人类工程团队犯一模一样的错 |
