...
接口和实现分家不算新鲜。真正值得追问的是再下一层：cloud agent 今天默认跑在一台 stateful 的 Linux 机器上——当文件系统都成了接口幻觉，"必须有一台带状态的机器"这个上一个时代的假设，是不是也该拆了。
...
它一口气挂上了二十多种 resource——S3 / GitHub / Gmail / Notion / Slack / Postgres / MongoDB / Linear / SSH / Discord——每一种都是一个 mount 点，操作动词全是 bash（完整 resource 列表见 [Mirage 官方文档](https://docs.mirage.strukto.ai)）。**Mirage 的核心赌注是：LLM 在训练分布上对 bash 和 Unix 工具的密度，远高于任意一个 SaaS SDK**。让 agent 用 SDK 等于让母语者讲外语；让它用 bash 等于把交互成本压到接近零。
...
把 Mirage 的 [`Workspace`](https://github.com/strukto-ai/mirage/blob/main/python/mirage/workspace/workspace.py) 类签名拉出来看：
...
`ConsistencyPolicy`、`history`、`index`、`session_id`、`agent_id`、`observe`、`snapshot()`——这些不是文件系统的词汇，是数据库的词汇。（这里是节选，真实签名还带着 `cache`、`mode`、`fuse` 等十来个参数，方向完全一致。）Mirage 给 agent 看的接口是 file/bash，但它管理 agent 自己的 state 用的是事务化、可观测、可快照的语义。**file 只是壳**。
