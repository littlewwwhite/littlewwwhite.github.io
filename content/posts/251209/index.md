---
title: "长期运行的 Agent 不需要更强的模型，需要更好的工程流程"
date: 2026-03-01
categories:
  - AI
  - Engineering
...
很多人认为让 Agent 构建复杂应用失败是因为模型还不够强。我的判断是：这不是 AI 能力问题，而是工程流程问题。

Anthropic 的工程师们花时间研究了如何让 Agent 在多轮 session 中有效协作，结论很有启发性——当把这个问题拆解成"工程师换班交接"的视角，答案就变得清晰了。

## 1. 典型失败模式：项目级的问题

让 Claude Opus 4.5 用一条指令"build a clone of claude.ai"，它会失败。但有趣的是，失败的模式和人类工程团队犯的错误一模一样：
