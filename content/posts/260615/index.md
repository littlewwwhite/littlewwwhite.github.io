...
下面只把它当成一份运行时切片来读。数据来自 [phistory](https://github.com/WEIFENG2333/phistory) 对 `@anthropic-ai/claude-code` 的归档。边界先说清楚：这是对实际运行时 prompt/工具协议的逆向归档，**不是官方产品路线图**；它能告诉你"能力边界怎么变了"，不能告诉你 Anthropic 内部怎么想。

我主要抓三条线：身份从 CLI 翻到 Agent，副作用从建议变成协议约束，会话从一次性对话长出记忆、调度、远程触发和编排。三条线合在一起，"runtime 化"这个判断才站得住。
...
不必逐字读 341 份 prompt。先看每个版本相对上一版的字节增量，突变点自己会跳出来。剔掉两类采集噪声后——`2.1.69` 是一份被截断的坏快照（19,678 字节，前后 ±60K 是假信号），`2.1.111`–`2.1.122` 在 ~105K 和 ~113K 之间反复横跳是条件性模块时有时无——有用的突变点就剩这几处，基本能把演化切成四段：
...
还有个反直觉的地方：**修改量最大的一次（`2.1.153 → 2.1.154`，净 -22KB）是删多于加**。它把一堆常驻工具描述挪进了按需加载的 `ToolSearch`，省下的体量比新加的还多。这个动作很说明问题：工具不再都塞在 prompt 里硬背，而是变成可延迟加载的协议。

## 委派被协议化：从无类型 Task 到确定性编排引擎

agent 化最硬的一条线是"委派"。1.0.0 的 Task 工具是无类型的，本质是个"帮我多搜几轮"的通用助手。`1.0.60` 给它加了必填参数：

> When using the Task tool, you must specify a subagent_type parameter to select which agent type to use.

一行约束，把"外包一次搜索"升级成"按类型派发具名子代理"。后面的多代理能力，基本都是从这儿长出来的。紧接着 `1.0.72` 补上 `run_in_background` + `BashOutput` + `KillBash`——agent 第一次能 spawn 一个长跑进程、轮询它、杀掉它，同时继续干别的活。"开个终端跑着、回头再看"这种并发习惯，被写进了工具协议。
...
它是一个用 `agent()` / `parallel()` / `pipeline()` / `phase()` / `budget()` 写的 JavaScript 编排引擎：按 cache 前缀可恢复（resume）、有 token 预算上限、有并发上限，还配了一个常驻的 Ultracode "默认就编排"模式。同一版本里，`SendMessage` 让子代理从"一次性 Task"变成"可续聊的执行单元"，`isolation: "worktree"` 给并行写文件的 agent 各自一个 git worktree。**以前靠人拆任务、调度、合并，现在开始变成可脚本化、可复现的运行时。**
...
agent 能跑命令、能写文件，风险就不只是"答错"，而是会真的"搞坏"。这一年里，副作用治理从"靠 agent 自觉"变成了"默认受约束"。
...
权限不再只是"逐条命令现批"，而是可以按语义、按最小特权、按会话提前说清楚。Observe → Plan → Approve → Act 这条链，被一颗一颗螺丝拧进了工具协议。
...
这就是这部演化史最深的一条线：**Claude Code 从"聪明的聊天框"变成了"受约束的执行系统"。**很多关键改动，都是把工程师原本临场做的判断，抽出来固化成运行时保证。身份从"官方 CLI"翻转成"Agent SDK 之上的 agent"，正是这一整套 runtime 化在自我定义上的盖章时刻——它先把自己重新定义成了 agent，然后才有资格、也才有必要，给自己装上这一圈约束。

我最在意的反而是 2.1.154 新加的这句：`Report outcomes faithfully: if tests fail, say so with the output`。早期 prompt 反复强调"少废话、别烦用户"；现在更要紧的是"别制造假确定性"。对一个会真的去改你代码库的执行系统来说，后者才是信任的地基。

## 抄作业：这部演化史，对自建 Agent 系统意味着什么

如果你也在做自己的 agent（我手上是宠物问诊和小说工具两套），这条 1.0.0 → 2.1.177 的轨迹里，有几条我会直接照着用：
