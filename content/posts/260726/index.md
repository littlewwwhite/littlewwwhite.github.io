---
title: "向量检索的两条降本路径：turbopuffer 的存储分层与 turbovec 的低比特量化"
date: 2026-07-26
categories:
- Embedding
- RAG
...
最近在给一个网页端 Agent 写小说产品设计长期记忆检索时，我同时看到了 turbopuffer 和 turbovec。最初，我把它们放进了同一份候选表：都处理向量，都强调性能和成本，看起来理应横向比较。

但继续读架构文档、项目说明和 TurboQuant 论文后，我发现候选表的维度从一开始就错了。turbopuffer 是搜索数据库，turbovec 是嵌入式压缩索引；一个重新安排数据跨越对象存储、SSD 与内存的路径，另一个重新安排向量在内存与 CPU 中的表示。

因此，真正值得比较的问题不是“谁更快”，而是：**当向量检索变贵时，成本究竟来自数据放得太贵，还是一条向量本身太大？**
...
- 两者不是云版与本地版的关系，也不是同一层的替代品。把它们放在一起的价值，是看清向量检索成本可以从哪两个正交方向被压低。
...
```mermaid
flowchart TB
    C["向量检索成本"] --> L["位置成本：跨越哪些存储层级"]
    C --> D["密度成本：每条向量多少字节"]
    L --> P["turbopuffer：对象存储 / 缓存 / 分片"]
    D --> V["turbovec：量化 / Bit Packing / SIMD"]
```
...
![向量检索的两个降本杠杆：改变数据所在层级，或减少每条向量的字节数](01-cost-levers.png "两个彼此独立的降本杠杆：位置与密度")
...
```mermaid
flowchart TB
    D["海量文档"] --> R["过滤 / BM25 / 向量召回"]
    R --> C["几十或几百个候选"]
    C --> X["RRF / 业务规则 / Cross Encoder / LLM"]
    X --> O["最终上下文"]
```
...
它接收向量、构建本地压缩索引，在压缩码上完成近似评分并返回 Top-K。
...
![turbovec 将 Float32 向量压成 2–4 bit，并直接在压缩码上评分](03-compressed-code-scoring.png "压缩减少搬运字节；查询无需逐向量恢复为 Float32")

它还支持把过滤下沉到搜索 kernel。例如，先由 SQL 或权限系统得到一组允许的 ID，再把 allowlist 交给 turbovec：

应用层先把允许的 ID 交给 turbovec，turbovec 再只在允许集合对应的压缩向量中评分并返回 Top-K。
...
选型时，我会先判断缺的是一个完整搜索系统，还是一个更省内存的本地向量内核：

```mermaid
flowchart TB
    A{"需要托管式分布式搜索吗？"} -->|是| P["优先评估 turbopuffer"]
    A -->|否| B{"单机内存或带宽是主要瓶颈吗？"}
    B -->|是| V["优先评估 turbovec"]
    B -->|否| E["先保留现有检索栈"]
    P --> PC["验证冷 / 热延迟与过滤召回"]
    V --> VC["验证 bit width、Recall@K 与索引生命周期"]
```
...
读完这两套设计后，我更倾向于把向量检索看成一条跨越存储层级与 CPU 寄存器的数据流，而不是一场 ANN 名词竞赛。turbopuffer 和 turbovec 恰好从两个方向重新审视了这条数据流。
