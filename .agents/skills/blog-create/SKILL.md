...
Generate a complete, ready-to-publish blog post from user's raw ideas, notes, and reference materials. User provides direction; Codex handles digestion, thinking, and writing.
...
1. Extract the image URL from the page DOM (via CDP `/eval` or HTML `<img>` tags)
...
After reading all material (and completing Step 1.7 if applicable), Codex MUST output a **thinking note** before writing. This is the critical step that prevents "rewrite syndrome."
...
| **失败→发现叙事** | Each section is a "tried X → failed → discovered Y" mini-arc. | Experience reports, tool evaluations, war stories | @trq212《Building Codex》 |
...
- **User focuses on ideas, Codex handles format**
